...
echo 'you have made an error, you entered the wrong input'
}
...
#Clean up function
function cleanup {
#rm -f *.tmp #remove temp files
rm /tmp/* &> /dev/null
echo 'exiting and cleaning temp files.'
exit 2
}
#trap command to run the clean up command
trap cleanup SIGHUP
